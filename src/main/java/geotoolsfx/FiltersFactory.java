...
public class FiltersFactory {
  ...
  public static void unique(FeatureCollectionWrapper featureCollection, String factoryName,
      boolean useFilter, List<String> attributes) {
  ...
  private static void getUniqueValuesFromTree(List<String> attributes, int attributeIndex,
      Entry<Object, Map> rootEntry, List<Map<String, Object>> uniqueValues) {
    ...
    if (!rootEntry.getValue().isEmpty()) {
      for (Object entry : rootEntry.getValue().entrySet()) {
        getUniqueValuesFromTree(attributes, attributeIndex + 1, (Entry<Object, Map>) entry,
            uniqueValues);
      }
    }
  }

  public static List<List<Map<String, Object>>> within(FeatureCollectionWrapper featureCollection,
      String factoryName, boolean useFilter, FeatureCollectionWrapper containerFeatureCollection,
    ...
    try {
      while (containerFeatures.hasNext()) {
        ...
        Filter filter = ff.within(ff.property("the_geom"),
            ff.literal(containerFeature.getDefaultGeometryProperty().getValue()));
        SimpleFeatureCollection containedCollection =
            featureCollection.getFeatures().subCollection(filter);
