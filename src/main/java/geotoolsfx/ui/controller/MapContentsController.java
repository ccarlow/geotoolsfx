...
public class MapContentsController implements Initializable, MapContentsListener {
  ...
  public void initialize(URL location, ResourceBundle resources) {
    treeView.getSelectionModel().getSelectedItems()
        .addListener(new ListChangeListener<TreeItem<String>>() {
          @Override
  ...
  public static MapGraphicController getMapGraphicController(MapContent mapContent) {
    ...
    FXMLLoader fxmlLoader = new FXMLLoader(MapContentsController.class.getClassLoader()
        .getResource("geotoolsfx/fxml/MapGraphic.fxml"));
    try {
  ...
  public void openMapLayers() {
    ...
    FXMLLoader fxmlLoader =
        new FXMLLoader(getClass().getClassLoader().getResource("geotoolsfx/fxml/Layers.fxml"));
    try {
      Node root = (Node) fxmlLoader.load();
      LayersController layersController = fxmlLoader.getController();
      MapContent mapContent = getMapContentFromTreeItem(treeItem);
      layersController.setMapContent(mapContent);
      DockPane dockPane = new DockPane(mapContent.getTitle() + " Layers", root);
      dockPane.setDockManager(mapContentsDockPane.getDockManager());
      dockPane.show();
    } catch (IOException e) {
      e.printStackTrace();
    }
